...
A Local First Auth mini app living at `apps/people` in this workspace, served under
`/people/` on the shared domain. Scaffolded from `templates/mini-app-starter` by
`pnpm new-app` — signup/login, REST API, SQLite (D1), and real-time WebSocket updates
are already wired.

## Everyday commands

From this directory (`apps/people`):
...
- The worker serves under the subpath — in dev the API is at
  `http://localhost:<worker-port>/people/api`, not `/api` (Hono `basePath`).
