...
import { decodeAndVerifyJWT } from '@dweb/people-shared'
...
interface AuthContextType {
  ...
  applyUser: (partial: Partial<User>) => void
  subscribeToWs: (listener: (message: { type: string; data?: any }) => void) => () => void
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const { resetMessage, setResetMessage, subscribe: subscribeToWs } = useWebSockets({
    userId: user?.did,
    isAdmin: user?.isAdmin ?? false,
    onReset: handleReset,
  })

  // Merge server-confirmed fields (e.g. after check-in) into the session user
  const applyUser = useCallback((partial: Partial<User>) => {
