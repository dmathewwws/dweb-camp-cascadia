# CLAUDE.md for People

This file provides guidance to Claude Code (claude.ai/code) when working with code in this app.

## Project Overview

A mini app living at `apps/people` inside a multi-app workspace, served under
`/people/` on the shared domain (Vite `base`, React Router `basename`, and Hono
`basePath('/people')` are already wired — client API calls are
...
Server endpoints verify JWTs using `decodeAndVerifyJWT` from `@dweb/people-shared`. The function validates the signature, expiration, and returns the typed payload. See `/shared/src/jwt.ts` for implementation.
...
```bash
pnpm wrangler d1 execute dweb-people-mini-app-dev-db --local --command "SELECT * FROM users;"
```

**Production (Remote D1):**
```bash
# Find the database name (format: dweb-people-mini-app-<stage>-db)
pnpm wrangler d1 list

# Run a query
pnpm wrangler d1 execute dweb-people-mini-app-prod-db --remote --command "SELECT * FROM users;"
```
...
The server mounts everything under the subpath (`basePath('/people')`), so the API
lives at `http://localhost:<worker-port>/people/api/...` — a bare `/api/...` request
returns 404. The Vite dev server proxies `/people/api` to the worker for you.