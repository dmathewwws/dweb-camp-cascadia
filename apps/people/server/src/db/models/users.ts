...
/**
 * Users visible in the directory: checked in and not blocked,
 * most recent check-in first
 */
export async function getDirectoryUsers(db: Database): Promise<User[]> {
  return await db
    .select()
    .from(users)
    .where(and(eq(users.blocked, false), isNotNull(users.interests)))
    .orderBy(desc(users.checkedInAt))
    .limit(1000)
}
