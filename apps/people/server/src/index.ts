...
const app = new Hono<{ Bindings: Env }>().basePath('/people')
...
/**
 * The user shape safe to expose to everyone (directory + broadcasts).
 * Strips blocked/isAdmin/socials — only the owner's own HTTP responses
 * may include isAdmin.
 */
...
app.post('/api/add-user', async (c) => {
  try {
    ...
    if (!user.blocked) {
      await notifyDO(c, 'user-joined', toPublicUser(user))
    }

    // The owner may see their own admin flag, but not other internal columns
    return c.json({ ...toPublicUser(user), isAdmin: user.isAdmin })
  } catch (error) {
...
app.post('/api/check-in', async (c) => {
  try {
    ...
    // Key the user off the cryptographically verified DID (see add-user)
    const payload = await verifyJwt(c, profileJwt)
    const did = payload.iss

    if (line != null && typeof line !== 'string') {
      return c.json({ error: 'line must be a string' }, 400)
    }
    ...
    if (!Array.isArray(interests) || interests.some((t) => typeof t !== 'string')) {
      return c.json({ error: 'interests must be an array of strings' }, 400)
    }
    const uniqueInterests = [...new Set(interests as string[])]
    if (uniqueInterests.length < MIN_INTERESTS || uniqueInterests.length > MAX_INTERESTS) {
      return c.json(
        { error: `pick between ${MIN_INTERESTS} and ${MAX_INTERESTS} interests` },
        400
      )
    }
    const unknown = uniqueInterests.filter((t) => !ALL_TAGS_SET.has(t))
    if (unknown.length > 0) {
      return c.json({ error: `Unknown interests: ${unknown.join(', ')}` }, 400)
    }
    ...
    if (!user.blocked) {
      await notifyDO(c, 'user-updated', toPublicUser(user))
    }
    ...
    return c.json(
      { error: 'Failed to check in', message: (error as Error).message },
      500
    )
  }
})
...
/**
 * GET /api/users - Get all checked-in, non-blocked users (the directory)
 */
app.get('/api/users', async (c) => {
  try {
    const db = createDb(c.env.DB)
    const users = await UserModel.getDirectoryUsers(db)
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === '/people/api' || url.pathname.startsWith('/people/api/')) {
    ...
    url.pathname = url.pathname.slice('/people'.length) || '/'
