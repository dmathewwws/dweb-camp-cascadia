...
## App-specific exception: the R2 presign credentials

`R2_ACCESS_KEY_ID` / `R2_SECRET_ACCESS_KEY` (plus the derived `R2_ACCOUNT_ID` /
`R2_BUCKET_NAME` bindings) intentionally **skip step 2** — they are not listed in
wrangler.toml `[secrets] required`. Their absence from `c.env` is what switches the
worker into dev mode for uploads: `/api/request-uploads` hands out worker-relative
`/api/dev-upload/*` URLs that write to wrangler's local simulated bucket instead of
presigned `r2.cloudflarestorage.com` URLs. Adding them to `[secrets]` would make
local dev try (and fail) to presign against the real bucket.
