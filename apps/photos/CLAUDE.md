# CLAUDE.md for Photos

This file provides guidance to Claude Code (claude.ai/code) when working with code in this app.

## Project Overview

A mini app living at `apps/photos` inside a multi-app workspace, served under
`/photos/` on the shared domain (Vite `base`, React Router `basename`, and Hono
`basePath('/photos')` are already wired — client API calls are
...
Server endpoints verify JWTs using `decodeAndVerifyJWT` from `@dweb/photos-shared`. The function validates the signature, expiration, and returns the typed payload. See `/shared/src/jwt.ts` for implementation.
...
```bash
pnpm wrangler d1 execute dweb-photos-mini-app-dev-db --local --command "SELECT * FROM users;"
```

**Production (Remote D1):**
```bash
# Find the database name (format: dweb-photos-mini-app-<stage>-db)
pnpm wrangler d1 list

# Run a query
pnpm wrangler d1 execute dweb-photos-mini-app-prod-db --remote --command "SELECT * FROM users;"
```
...
The server mounts everything under the subpath (`basePath('/photos')`), so the API
lives at `http://localhost:<worker-port>/photos/api/...` — a bare `/api/...` request
returns 404. The Vite dev server proxies `/photos/api` to the worker for you.